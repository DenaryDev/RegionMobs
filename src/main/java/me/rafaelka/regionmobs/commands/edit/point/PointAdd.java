...
public class PointAdd extends Point {
    ...
    @Override
    public String info() {
        return Settings.messages().commands.edit.point.add.info;
    }
    ...
    protected ArgumentBuilder<CommandSourceStack, ?> subCommand() {
        return root().executes(context -> {
            ...
            if (region.hasPoint(point)) {
                sendMessage(source, Settings.messages().commands.edit.point.add.already);
                return 1;
            }
            ...
            sendMessage(source, Settings.messages().commands.edit.point.add.success,
